...
Manual labelling. Hand-label a stratified sample of 30–50 filtered events
(low / medium / high) and compare to the model's badge; investigate every
disagreement.
Distribution check. Plot the score histogram. If almost everything lands
in one tier, re-tune the weights/thresholds in the config.
Low-variance guard (a standing rule). Any component where > 90% of events
...
Coordinate-coverage check. Report the share of events with usable
`lat`/`long`; the more that fall back to borough-level transit, the lower the
confidence — state this number in the final report.
Future component rule. Only introduce a separate proximity / hyperlocal
